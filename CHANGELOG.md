...
- `mPhpMaster\BuilderFilter\QueryBuilder` class no longer extends Laravel's `Illuminate\Database\Eloquent\Builder`
...
- resolve `BuilderFilterRequest` from service container
...
- renamed `mPhpMaster\BuilderFilter\Sort` to `mPhpMaster\BuilderFilter\AllowedSort`
- renamed `mPhpMaster\BuilderFilter\Included` to `mPhpMaster\BuilderFilter\AllowedInclude`
- renamed `mPhpMaster\BuilderFilter\Filter` to `mPhpMaster\BuilderFilter\AllowedFilter`
...
- bugfix: replace missing `sort()` method on `BuilderFilterRequest`
...
- started using `BuilderFilterRequest` to read data from the current request
